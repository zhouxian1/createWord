"""知识库API - 支持上传、API对接、批量导入、审核、增量导入、版本回滚"""
import os
import uuid
import json
import hashlib
...
def upload_file(kb_id):
    ...
    for file in files:
        ...
        result = _process_uploaded_file(file, kb_id, upload_dir, kb)
...
def batch_import(kb_id):
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            # 计算哈希
            kf.file_hash = _compute_file_hash(new_path)
...
def _process_uploaded_file(file, kb_id, upload_dir, kb) -> dict:
    """处理上传的文件"""
