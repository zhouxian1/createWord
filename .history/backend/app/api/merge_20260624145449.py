...
def delete_merge_task(task_id):
    ...
    storage = get_storage()
    if task.output_path:
        if not os.path.isabs(task.output_path):
            storage.delete_file(task.output_path)
...
def download_file(filename):
    ...
    if storage.available and storage.file_exists(object_name):
        # 下载到临时文件
        with tempfile.NamedTemporaryFile(suffix='.docx', delete=False) as tmp:
            tmp_path = tmp.name
        if storage.download_file(object_name, tmp_path):
...
def download_document(doc_id):
    ...
    if document.file_path:
        ...
        if not os.path.isabs(document.file_path):
            with tempfile.NamedTemporaryFile(suffix='.docx', delete=False) as tmp:
                tmp_path = tmp.name
            if storage.download_file(document.file_path, tmp_path):
