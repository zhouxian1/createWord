...
class DocumentParser:
    ...
    def _parse_doc(self, file_path):
        """解析DOC文档（旧格式）- 尝试多种方式"""
        # 方式1: 尝试用docx2txt（有些.doc实际是docx格式）
        try:
            import docx2txt
        ...
        # 方式2: 尝试用python-docx（有些.doc实际是docx格式）
        try:
            return self._parse_docx(file_path)
        except Exception:
            pass

        # 方式3: 使用olefile提取OLE文档中的文本
        try:
            import olefile
            if olefile.isOleFile(file_path):
                ...
                if ole.exists('WordDocument'):
                    ...
                    # 从WordDocument流中提取文本（简化方法）
                    # FIB (File Information Block) 前32字节
                    import struct
                    # 读取文本位置信息
                    if len(word_stream) > 0x004C:
                        ccpText = struct.unpack_from('<I', word_stream, 0x004C)[0]
                        # 尝试从主文档流中提取Unicode文本
                        if ccpText > 0 and len(word_stream) > ccpText * 2:
                            # Unicode文本通常在文档流末尾
                            text_bytes = word_stream[-ccpText*2:]
                            try:
                                text = text_bytes.decode('utf-16-le', errors='ignore')
                                text = text.strip('\x00').strip()
                                if text and len(text) > 50:
                                    ole.close()
                                    return {'text': text, 'structure': {'type': 'doc', 'method': 'olefile'}}
                            except Exception:
                                pass
                ole.close()
        except Exception:
        ...
        # 方式4: 使用antiword命令行工具（Linux）
        try:
            import subprocess
            result = subprocess.run(['antiword', file_path], capture_output=True, text=True, timeout=30)
            if result.returncode == 0 and result.stdout.strip():
                return {'text': result.stdout, 'structure': {'type': 'doc'}}
        except Exception:
            pass

        # 方式5: 使用LibreOffice转换（如果安装了）
        try:
            import subprocess
            import tempfile
            ...
            if result.returncode == 0:
                import os
                base_name = os.path.splitext(os.path.basename(file_path))[0] + '.docx'
                converted_path = os.path.join(tmp_dir, base_name)
                if os.path.exists(converted_path):
                    parsed = self._parse_docx(converted_path)
        ...
        try:
            ...
            text = re.sub(r'\s{3,}', '\n', text)
            if text.strip():
                return {'text': text.strip(), 'structure': {'type': 'doc', 'method': 'binary_extract'}}
        except Exception:
            pass

        return {'text': '', 'structure': {'type': 'doc'}, 'error': '无法解析DOC格式文件，建议转换为DOCX格式'}
