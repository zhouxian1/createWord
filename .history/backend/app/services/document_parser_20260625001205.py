...
class DocumentParser:
    ...
    def _parse_doc(self, file_path):
        """解析DOC文档（旧格式）- 尝试多种方式"""
        # 方式1: 尝试用python-docx（有些.doc实际是docx格式）
        try:
        ...
        # 方式3: Windows环境使用win32com转换
        try:
            import subprocess
            import tempfile
            import os
            # 使用PowerShell + Word COM自动化转换DOC为DOCX
            ...
            if os.path.exists(docx_path):
                parsed = self._parse_docx(docx_path)
                os.unlink(docx_path)
        ...
        return {'text': '', 'structure': {'type': 'doc'}, 'error': '无法解析DOC格式文件，建议转换为DOCX格式或安装LibreOffice'}
