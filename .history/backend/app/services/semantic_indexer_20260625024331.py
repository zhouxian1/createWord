...
class SemanticIndexer:
    ...
    def __init__(self, db_path=None, embedding_model=None, chroma_host=None, chroma_port=None):
        ...
        try:
            self._chroma_host = chroma_host or current_app.config.get('CHROMA_HOST', '')
        except RuntimeError:
            self._chroma_host = chroma_host or os.environ.get('CHROMA_HOST', '')
        try:
            self._chroma_port = chroma_port or current_app.config.get('CHROMA_PORT', 8000)
        except RuntimeError:
    ...
    def _get_collection(self, collection_name='documents'):
        """获取或创建向量集合（优先使用ChromaDB Server，回退到嵌入式PersistentClient）"""
        # 如果已确认不可用，直接返回
        if self._chroma_available is False:
            return None
        if self._chroma_available is True and self._collection is not None:
            return self._collection
        try:
            import chromadb

            # 优先尝试ChromaDB Server模式
            if self._chroma_host:
                try:
                    ...
                    logger.info(f"ChromaDB Server模式连接成功: {self._chroma_host}:{self._chroma_port}")
                    return self._collection
                except Exception as e:
                    logger.warning(f"ChromaDB Server连接失败({self._chroma_host}:{self._chroma_port}): {str(e)}，回退到嵌入式模式")

            # 嵌入式PersistentClient模式（带超时检测）
            import threading
            result = [None]
            error = [None]
            ...
            self._chroma_available = True
            logger.info("ChromaDB嵌入式模式初始化成功")
            return self._collection
        except ImportError:
