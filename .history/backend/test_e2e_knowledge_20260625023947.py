...
timer = threading.Timer(90, timeout_handler)
timer.daemon = True
timer.start()
...
_test_indexer = SemanticIndexer()
print(f"chromadb状态: {_test_indexer._chroma_available}")
...
with app.app_context():
    ...
    if file_paths:
        print(f'  导入文件: {[os.path.basename(p) for p in file_paths]}')
        ...
        if r.status_code == 200:
            ...
            print(f'  ERROR: status={r.status_code}, body={r.data[:200]}')
    else:
        print('  无docx文件可导入')
