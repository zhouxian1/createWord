<template>
  <div class="intelligent-qa tech-page">
    <el-card class="qa-hero">
      <div class="qa-hero__content">
        <div>
          <div class="qa-hero__tag">MULTIMODAL QA CONSOLE</div>
          <h2>智能问答与 438C 规范助手</h2>
          <p>支持知识库问答、438C 标准专项问答、答案溯源和语音输入，帮助你更快定位规范条款与项目内容。</p>
        </div>
        <div class="qa-hero__status">
          <div class="status-item">
            <span>语音输入</span>
            <strong>{{ speechSupported ? '可用' : '当前浏览器不支持' }}</strong>
          </div>
          <div class="status-item">
            <span>会话消息</span>
    ...
    <el-row :gutter="20">
                ...
                <div class="message-content">
                  ...
                  <div v-if="msg.source_traces && msg.source_traces.length" class="source-traces">
                    ...
                    <div v-for="(trace, ti) in msg.source_traces" :key="ti" class="trace-item">
                      ...
                      <span class="trace-confidence">置信度: {{ (trace.confidence * 100).toFixed(1) }}%</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <el-col :span="8">
        <el-card>
          ...
          <p class="qa-desc">针对GJB 438C军用软件文档编制规范的专项问答</p>
