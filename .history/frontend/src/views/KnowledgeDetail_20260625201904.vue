<template>
  <div class="knowledge-detail cyber-page circuit-bg">
    ...
    <el-row :gutter="20" style="margin-top: 20px">
      <el-col :span="16">
        <el-card>
          ...
          <el-table :data="filteredFiles" stripe>
            ...
            <el-table-column prop="chunk_count" label="分块" width="55" />
            <el-table-column label="密级" width="65">
            ...
            <el-table-column label="索引" width="70">
              <template #default="{ row }">
                <el-tag :type="indexStatusType(row.index_status)" size="small">{{ indexStatusLabel(row.index_status) }}</el-tag>
              </template>
            </el-table-column>
      ...
      <el-col :span="8">
        ...
        <el-card style="margin-top: 20px">
          ...
          <el-input v-model="graphQuery" placeholder="输入实体名称，留空查看全量" @keyup.enter="loadGraph">
            <template #append>
...
const doApiImport = async () => {
  ...
  try {
    ...
    const res = await knowledgeApi.apiImport(kbId, {
      ...
      headers,
      body,
      fields: apiFields.value.split(',').map(f => f.trim()).filter(Boolean)
    })
    ElMessage.success(`API导入完成: ${res.imported_count || 0}条`)
    showApiImportDialog.value = false
    loadData()
  } catch { ElMessage.error('API导入失败') }
  finally { importing.value = false }
}
