<template>
  <div class="document-generation cyber-page circuit-bg">
    <div class="gen-hero holo-panel">
      <div class="hero-row">
        <div>
          <div class="cyber-badge">438C DOC GEN</div>
          <h2 class="glitch-title" style="margin-top: 10px; font-size: 22px;">文档生成</h2>
        </div>
    ...
    <el-card>
      ...
      <el-table :data="documents" stripe>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            <el-button size="small" text type="primary" @click="$router.push(`/generation/${row.id}`)">
              编辑
            </el-button>
            <el-button size="small" text type="success" @click="generateFull(row)" :disabled="row.status === 'generating'">
              生成
            </el-button>
            <el-button size="small" text @click="exportDoc(row)" :disabled="row.status !== 'generated' && row.status !== 'validated'">
              导出
            </el-button>
