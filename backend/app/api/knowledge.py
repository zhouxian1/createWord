...
def batch_import(kb_id):
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            logger.warning(f"文件上传失败，继续处理: {str(e)}")
            upload_result = {'storage_type': 'failed'}
        ...
        kf = KnowledgeFile(
            ...
            file_path=file_path,  # 存储原始文件路径作为回退
        ...
        try:
            ...
            # 计算哈希
            kf.file_hash = _compute_file_hash(file_path)
...
def get_knowledge_graph():
    """获取知识图谱，支持按实体名称查看关联子图"""
    entity_name = request.args.get('entity_name', '').strip()
    depth = request.args.get('depth', default=2, type=int)

    graph = get_indexer().knowledge_graph
    if entity_name:
        data = graph.search_related(entity_name, depth=max(1, min(depth, 4)))
        data['query'] = entity_name
        data['depth'] = depth
        return jsonify(data)
